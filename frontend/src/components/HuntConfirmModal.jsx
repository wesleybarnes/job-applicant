  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)' }}>
      <div className="rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/10" style={{ background: 'linear-gradient(180deg, #1a1625, #12101c)' }}>
        ...
        <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between" style={{ background: 'rgba(139,92,246,0.08)' }}>
          <div>
        ...
        <div className="p-6 space-y-4 max-h-[55vh] overflow-y-auto">
          {screenshot && <div className="rounded-2xl overflow-hidden border border-white/10 bg-black"><img src={`data:image/png;base64,${screenshot}`} alt="Application state" className="w-full object-contain max-h-48" /></div>}
          <div>
        ...
        <div className="px-6 py-4 border-t border-white/10 grid grid-cols-3 gap-3" style={{ background: 'rgba(0,0,0,0.3)' }}>
          <button onClick={onStop} className="flex items-center justify-center gap-1.5 bg-red-600 hover:bg-red-700 text-white py-2.5 px-3 rounded-xl font-semibold text-sm transition-colors"><Square className="w-3.5 h-3.5" /> Stop Hunt</button>
          <button onClick={onSkip} className="flex items-center justify-center gap-1.5 bg-white/8 hover:bg-white/12 text-zinc-300 py-2.5 px-3 rounded-xl font-semibold text-sm border border-white/10 transition-colors"><SkipForward className="w-3.5 h-3.5" /> Skip Job</button>
