...
export default function JobsPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto animate-fade-in">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-ink-primary tracking-tight">Find Jobs</h1>
        <p className="text-ink-secondary mt-2 text-base">Search listings and add them to your application queue.</p>
      </div>
      ...
      <div className="card p-5 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input className="input pl-10" placeholder="Job title, skills, keywords..." value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          </div>
          <div className="relative sm:w-52">
            <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-tertiary" />
            <input className="input pl-10" placeholder="City or Remote" value={location} onChange={e => setLocation(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          </div>
          ...
          <button onClick={() => setShowManual(v => !v)} className="btn-secondary whitespace-nowrap">
            {showManual ? <X className="w-4 h-4" /> : <Plus className="w-4 h-4" />} {showManual ? 'Cancel' : 'Add Manually'}
          </button>
        </div>
      ...
      {showManual && (
        <div className="card p-6 mb-6">
          <h3 className="font-bold text-ink-primary mb-4">Add Job Manually</h3>
      ...
      {loading ? (
        ...
        <div className="card p-20 text-center">
          <Building2 className="w-12 h-12 mx-auto mb-3 text-ink-tertiary opacity-20" />
          <p className="font-bold text-ink-primary">No jobs yet</p>
          <p className="text-sm text-ink-secondary mt-1">Search above or add a job manually.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {jobs.map(job => (
            <div key={job.id} className="card p-5 hover:shadow-card-hover transition-all duration-300">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h3 className="font-bold text-ink-primary text-sm">{job.title}</h3>
                    {job.remote_type && <span className={`badge text-xs ${job.remote_type === 'remote' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-600'}`}>{job.remote_type}</span>}
                    {job.match_score >= 70 && <span className="badge bg-brand-50 text-brand-600 text-xs">{Math.round(job.match_score)}% match</span>}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-ink-tertiary">
                    ...
                    {(job.salary_min || job.salary_max) && <span className="flex items-center gap-1"><DollarSign className="w-3 h-3" />{job.salary_min ? `$${(job.salary_min/1000).toFixed(0)}k` : ''}{job.salary_min && job.salary_max ? '–' : ''}{job.salary_max ? `$${(job.salary_max/1000).toFixed(0)}k` : ''}</span>}
                    {job.job_type && <span>{job.job_type}</span>}
                  </div>
                  {job.description && <p className="text-ink-tertiary text-xs mt-2 line-clamp-2">{job.description}</p>}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {job.url && <a href={job.url} target="_blank" rel="noopener noreferrer" className="btn-ghost text-xs py-1.5 px-3 border border-surface-border rounded-xl"><ExternalLink className="w-3 h-3" /> View</a>}
                  <button onClick={() => handleApply(job.id)} disabled={addedJobs.has(job.id)} className={`text-xs py-2 px-4 rounded-xl font-semibold transition-all ${addedJobs.has(job.id) ? 'bg-emerald-50 text-emerald-700 cursor-default' : 'btn-primary'}`}>
                    {addedJobs.has(job.id) ? '✓ Added' : 'Add to Queue'}
                  </button>
                  <button onClick={() => handleDelete(job.id)} className="p-2 text-ink-tertiary hover:text-red-500 rounded-xl transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            </div>
          ))}
