    ...
    return (
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {form[field].map(v => (
            <span key={v} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-500/10 text-brand-400 text-xs font-medium border border-brand-500/20">
              {v}<button onClick={() => removeFromList(field, v)} className="hover:text-red-400 ml-0.5"><X className="w-3 h-3" /></button>
            </span>
          ))}
        </div>
      </div>
    ...
    <div className="flex flex-wrap gap-2">
      {options.map(opt => (
        <button key={opt} type="button" onClick={() => onChange(opt)}
          className={`px-4 py-2 rounded-xl text-sm font-medium border transition-all duration-200 ${value === opt ? 'bg-brand-500/15 text-brand-400 border-brand-500/30' : 'bg-surface-card text-ink-secondary border-surface-border hover:border-brand-500/30'}`}>
          {opt}
        </button>
      ))}
    </div>
    ...
    <div className="min-h-screen flex flex-col bg-surface-bg">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-xl">
          <div className="mb-8">
            <div className="h-1 bg-surface-border rounded-full overflow-hidden mb-6">
              <div className="h-full bg-gradient-to-r from-brand-500 to-brand-400 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
            <div className="text-center">
              <p className="text-xs text-brand-400 font-bold uppercase tracking-wider mb-1">Step {step} of {STEPS.length}</p>
          ...
          <div className="card p-8">
            {error && <div className="mb-5 p-3 bg-red-500/10 text-red-400 rounded-xl text-sm border border-red-500/20">{error}</div>}
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div><label className="label">LinkedIn</label><input className="input" value={form.linkedin_url} onChange={e => set('linkedin_url', e.target.value)} placeholder="linkedin.com/in/jane" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="label">GitHub</label><input className="input" value={form.github_url} onChange={e => set('github_url', e.target.value)} placeholder="github.com/jane" /></div>
                  <div><label className="label">Portfolio</label><input className="input" value={form.portfolio_url} onChange={e => set('portfolio_url', e.target.value)} placeholder="jane.dev" /></div>
                </div>
              </div>
            )}
            {step === 2 && (
              <div className="space-y-5">
                ...
                <div><label className="label">Locations</label><TagInput field="target_locations" placeholder="e.g. Tokyo" /></div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="label">Remote</label><ChipSelect options={REMOTE_OPTIONS} value={form.remote_preference} onChange={v => set('remote_preference', v)} /></div>
                <div><label className="label">Work Authorization</label><select className="input" value={form.work_authorization} onChange={e => set('work_authorization', e.target.value)}><option value="">Select...</option>{WORK_AUTH_OPTS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                <div><label className="label">Availability</label><ChipSelect options={AVAILABILITY_OPTS} value={form.availability} onChange={v => set('availability', v)} /></div>
                <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={form.willing_to_relocate} onChange={e => set('willing_to_relocate', e.target.checked)} className="w-4 h-4 accent-brand-500 rounded" /><span className="text-sm text-ink-primary">Willing to relocate</span></label>
              </div>
            )}
            {step === 3 && (
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><label className="label">Education</label><select className="input" value={form.education_level} onChange={e => set('education_level', e.target.value)}><option value="">Select...</option>{EDUCATION_OPTS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                </div>
                <div><label className="label">Skills</label><TagInput field="skills" placeholder="e.g. Python, React" /></div>
                <div><label className="label">Summary</label><textarea className="input min-h-[120px] resize-y" value={form.summary} onChange={e => set('summary', e.target.value)} placeholder="Your experience and goals..." /></div>
              </div>
            )}
            {step === 4 && (
              <div className="space-y-4">
                <p className="text-sm text-ink-secondary">Upload your resume so the AI can fill applications accurately.</p>
                <label className={`flex flex-col items-center justify-center border border-dashed rounded-2xl p-12 cursor-pointer transition-all ${dragOver ? 'border-brand-500/50 bg-brand-500/5' : resumeFile ? 'border-brand-500/50 bg-brand-500/5' : 'border-surface-border hover:border-brand-500/30'}`}
                  ...
                  <Upload className={`w-10 h-10 mb-3 ${resumeFile ? 'text-brand-400' : 'text-ink-tertiary'}`} />
                  {resumeFile ? (<><span className="font-semibold text-ink-primary">{resumeFile.name}</span><span className="text-xs text-ink-tertiary mt-1">{(resumeFile.size / 1024).toFixed(0)} KB</span></>) : (<><span className="font-medium text-ink-primary">Click or drop</span><span className="text-xs text-ink-tertiary mt-1">PDF, DOCX, TXT · Max 10MB</span></>)}
                  <input type="file" className="hidden" accept=".pdf,.docx,.doc,.txt" onChange={e => setResumeFile(e.target.files?.[0] || null)} />
                </label>
                {resumeFile && <button type="button" onClick={() => setResumeFile(null)} className="text-sm text-red-400 hover:text-red-300 font-medium">Remove</button>}
              </div>
            )}
            {step === 5 && (
              <div className="space-y-5">
                <p className="text-sm text-ink-secondary">Pre-loaded answers for common questions.</p>
                {[
                  { key: 'tell_me_about_yourself', label: 'Tell me about yourself', ph: 'Background and goals...' },
                  ...
                  { key: 'why_leaving', label: 'Why looking for a new role?', ph: 'Growth, new challenges...' },
                ].map(({ key, label, ph }) => (
                  <div key={key}><label className="label">{label}</label><textarea className="input min-h-[80px] resize-y" value={form.custom_answers[key]} onChange={e => set('custom_answers', { ...form.custom_answers, [key]: e.target.value })} placeholder={ph} /></div>
                ))}
              </div>
            )}

            <div className="flex justify-between mt-8 pt-6 border-t border-surface-border">
              <button type="button" onClick={() => setStep(s => s - 1)} disabled={step === 1} className="btn-ghost flex items-center gap-2 disabled:invisible"><ChevronLeft className="w-4 h-4" /> Back</button>
              {step < STEPS.length
                ? <button type="button" onClick={() => setStep(s => s + 1)} disabled={!canProceed()} className="btn-primary flex items-center gap-2 disabled:opacity-40">Continue <ChevronRight className="w-4 h-4" /></button>
                : <button type="button" onClick={handleSubmit} disabled={loading || !canProceed()} className="btn-primary flex items-center gap-2 disabled:opacity-40">
                    {loading ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Setting up...</> : <><Sparkles className="w-4 h-4" />Launch</>}
                  </button>
              }
            </div>
          </div>
